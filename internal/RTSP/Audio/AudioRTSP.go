...
const recordType = "audioRTSP"

type AudioRTSP struct {
...
func InitAudioRTSP(ctx context.Context, wg *sync.WaitGroup,
...
func (r *AudioRTSP) recordingProccess(wg *sync.WaitGroup,
	startSignal, done <-chan interface{}) {
	defer wg.Done()
	defer close(r.statusChannel)
	defer r.file.Close()
	defer close(r.repeatSignal)
...
func (r *AudioRTSP) startRunPipeline(wgStream *sync.WaitGroup, done <-chan interface{}) {
	defer wgStream.Done()
...
func (r *AudioRTSP) runPipeline(loop *glib.MainLoop) error {
...
func (r *AudioRTSP) runMonitoring(wgStream *sync.WaitGroup, done <-chan interface{}) {
	defer wgStream.Done()
...
func (r *AudioRTSP) buildPipelineString() string {
	r.count++

	filename := fmt.Sprintf("%s_%s_%d.mkv", r.Name, recordType, r.count)

	pipeline := strings.Join([]string{
		"rtspsrc", "location=" + r.Url, "protocols=tcp", "latency=200",
		"!", "queue", "!", "rtpmp4gdepay", "!", "aacparse", "!",
		"matroskamux", "!", "filesink", "name=fsink",
...
func (r *AudioRTSP) createPipeline() error {
...
func (r *AudioRTSP) updateState(state string, err string) {
	r.stateMutex.Lock()
	defer r.stateMutex.Unlock()
	r.state = general.MonitoringState{State: state, Errors: err, Significant: r.significant}
}

func (r *AudioRTSP) writeStatusFile(timestamp string) error {
	filename := fmt.Sprintf("%s_%s_%d.mkv", r.Name, recordType, r.count)
...
func (r *AudioRTSP) SendMonitoringInfo() <-chan general.MonitoringState {
